...
//		'\n\n');
	...
	let	op;
	let operate_sections = false;
	let reroute_pattern = /((?:after\s*(?:\[(?:Qs|Mn|Bx|Bk|SI)[0-9]{1,5}\-[A-z0-9]{1,5}\])\s*,?\s*)?(Some)?\s*(Northbound|Southbound|Uptown|Downtown|(?:\[(?:Qs|Mn|Bx|Bk|SI)[0-9]{1,5}\-[A-z0-9]{1,5}\])\s*[\s-]\s*bound|\b[^\[\]\`]*\b[\s-]bound)?\s*\[([A-Z0-9]{1,2})\]\s*(?:express|local)?(?:\*|\s)*(?:(?:\s|and)*\[([A-Z0-9]{1,2})\]\s*(?:express|local)?)?\s*(Northbound|Southbound|Uptown|Downtown|(?:\[(?:Qs|Mn|Bx|Bk|SI)[0-9]{1,5}\-[A-z0-9]{1,5}\])\s*[\s-]\s*(?:bound)?|\b[^\[\]\`]*\b[\s-]bound)?\s*(?:(?:([^`\[\]]*\b(?:No|service|trains)\s*(?:service|operates?|run)\s*b\s*etween\b|\[__operates-section-[0-9]__\]\s*(?:between)?)\s*(\[(?:[A-Z]{2}[A-Z0-9]{1,4}\-[A-Z0-9]{2,5}[|]?)+\])[^\[\]`]*(\[(?:[A-Z]{2}[A-Z0-9]{1,4}\-[A-Z0-9]{2,5}[|]?)+\])\,?)|(?:trains)?\s*(?:are\s*rerouted)?)?[^\[\]`]*?)((?:\s*(?:(?:and|then|run(?:ning)?|make|making|stopping)?\s*)+(?:(?:(on|via|a?long|over|replace)+\s*(?:the)?\s*\[((?!\3\4)[A-Z0-9])\]|(?:on\s*the\s*)?(express|local)(?:\s*track)?)[^\/\[\]`]*(express|local|to\s*\/\s*from|to|in|\[(?:[A-Z]{2}[A-Z0-9]{1,4}\-[A-Z0-9]{2,5}[|]?)+\])[^\[\]`]*(Manhattan|Queens|Brooklyn|the\s* Bronx|\[(?:[A-Z]{2}[A-Z0-9]{1,4}\-[A-Z0-9]{2,5}[|]?)+\])|to\s*\/\s*from\s*(?:the)?\s*(\[(?:[A-Z]{2}[A-Z0-9]{1,4}\-[A-Z0-9]{2,5}[|]?)+\])\s*\[([A-Z0-9]{1,2})\]\s*(?:station)?\s*(?:after\s*(\[(?:[A-Z]{2}[A-Z0-9]{1,4}\-[A-Z0-9]{2,5}[|]?)+\]))?))\s*(?:\s*\(skipping.*\)\s*|\,\s*the\s*last\s*stop|\,\s*then\s*end)?[\.,\s]*)((?:(?:(?:and|then)?\s*(?:trains\s*(?:run)\s*)?(?:stopping|run|operat(?:e|ing))?\s*(via|along|over|replace|on)+ the)\s*(\[(?!\3\4)[A-Z0-9]\])?[^\[\]`]*?((?:and\s*)?end(?:ing)?\s*at|to\s*\/\s*from|\bto\b|\[(?:[A-Z]{2}[A-Z0-9]{1,4}\-[A-Z0-9]{2,5}[|]?)+\])(?:[^\[\]`]*?(?:(\[(?:[A-Z]{2}[A-Z0-9]{1,4}\-[A-Z0-9]{2,5}[|]?)+\])[\.]?))?)?)/i;
	let no_trains_between_pattern = /(?:Service\s*is\s*(suspended)\s*(?:in\s*both\s*directions\s*)(?:on\s*the\s*)|(No)\s*)?\[([A-Z0-9]{1,2})\](?:\*|\s)*(?:(?:\s|and|or)*\[([A-Z0-9]{1,2})\])?\s*(?:train[s]?\s*(?:service)?\s*)?(?:(?:There\s*(?:is|will\s*be)\s*)?(No)?\s*(?:trains|service)\s*between|(?:line\s*)?between)\s*(?:(\[(?:[A-Z]{2}[A-Z0-9]{1,4}\-[A-Z0-9]{2,5}[|]?)+\])\s*(?:and)?\s*(\[(?:[A-Z]{2}[A-Z0-9]{1,4}\-[A-Z0-9]{2,5}[|]?)+\]))/i;
	let bypass_pattern = /((Some)?\s*(Northbound|Southbound|Uptown|Downtown|(?:\[(?:Qs|Mn|Bx|Bk|SI)[0-9]{1,5}\-[A-z0-9]{1,5}\])\s*[\s-]\s*bound|\b[^\[\[\`]*\b[\s-]bound)?\s*\[([A-Z0-9]{1,2})\](?:\*|\s)*(?:(?:\s|and)*\[([A-Z0-9]{1,2})\]\s*(?:express|local)?)?\s*(Some)?\s*(Northbound|Southbound|Uptown|Downtown|(?:\[(?:Qs|Mn|Bx|Bk|SI)[0-9]{1,5}\-[A-z0-9]{1,5}\])\s*[\s-]\s*bound|\b[^\[\]\`]\b[\s-]bound)?\s*(?:(?:trains|service)\s*(?:are|is|will)?\s*(skip(?:ping|s)?|bypass(?:ing|es)?|end(?:ing)?\s*(?:at|after))|(No\s*(?:trains|service))\s*at)\s*((?:\[(?:(?:Qs|Mn|Bx|Bk|SI)[0-9]{1,5}\-[A-z0-9]{1,5}[|]?)+\]\s*(?:,|and|or)*\s*)+))\s*((?:(?:trains|service)\s*(?:are|is)?\s*(skip(?:ping|s)?|bypass(?:ing|es)?)|(No\s*(?:trains|service))\s*at)\s*((?:\[(?:(?:Qs|Mn|Bx|Bk|SI)[0-9]{1,5}\-[A-z0-9]{1,5}[|]?)+\]\s*(?:,|and)*\s*)+))?/i;
	let operate_sections_pattern = /\[([A-Z0-9])\](?:(?:\s|and|\*)*\[([A-Z0-9])\])?\s*(?:(?:(?:(?:\[__operates-section-([0-9])__\]\s*|(?:(?:shuttle)?\s*(?:service|trains))\s*(?:are)?\s*(?:operate(?:s)?|operating|run(?:ning)?)\s*)+)(?:(?:(?:at)?\s*all\s*times)|weekend(?:s)?\s*(?:service)?)?\s*(?:(?:in)?\s*(?:Manhattan|Brooklyn|Queens|(?:the\s*)?Bronx))?\s*(?:between)?)\s*(\[(?:[A-Z]{2}[A-Z0-9]{1,4}\-[A-Z0-9]{2,5}[|]?)+\])(?:[^\[\]`]|\[[a-z0-9]\])*(\[(?:[A-Z]{2}[A-Z0-9]{1,4}\-[A-Z0-9]{2,5}[|]?)+\]))+/i;
	let module = [
		{
			id: 'bypass_stations',
		...
		regex_match.map((item, i) => {
			...
			switch (i) {
				...
				case 19:
					...
					// A run via B from 1 to 2, then end.
					// A run via B from 1 and end at 2.
					// A run via B from 1 to 2, the last stop.
					let endAtPattern = /((?:then|and)\s*end\s*(?:at)?|\,\s*the\s*last\s*stop|\,\s*then\s*end)+/ig
				...
				case 23:
					...
					else if (i == 15 && afterStationPrefix) {
							// Either:
							// After [Mn167-A32], some [D] trains will run along the [F] line to [Bk58-D43].
							route_pair.route[j].from = afterStationPrefix;
					}
